...
as well. I have begun to implement code which should allow us to control the quads roll, pitch, and yaw. This will in turn lead to the programming necesarry for the microcontroller to send translational and rotation commands to the motors during the descent.
...
Overall our current status is that we have an operational if not flight capable airframe. The navigation package has been prototyped and tested and is in working order, with the exception of the failure of our current microcontroller due to unknown reasons.


##Analysis

Below is a graph of the thrust to weight ratio of the combined efforts of our motor as a function of load weight colored by Throttle Percentage. This is usefull to information because in order to maintain flight the quad must have a thrust to weight ratio of at least 2. From the graph we can see that we our quad should weigh no more than 680 pounds in order to have a thrust to weight ratio of 2 or more for 60 percent of full throttle.
...
battlife<-(1.3/(amps.per.motor*4)*.7)
